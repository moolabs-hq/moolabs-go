/*
Moolabs API

Unified Moolabs SDK. Capability-based public surface stitched from internal service specs: bff, meter, arc, acute. Customers see capability namespaces (usage, billing, collections, etc.); the SDK routes per-operation internally.

API version: 0.2.0-rc10
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package moolabs

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the SubscriptionExpanded type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &SubscriptionExpanded{}

// SubscriptionExpanded Expanded subscription
type SubscriptionExpanded struct {
	// A unique identifier for the resource.
	Id string `json:"id" validate:"regexp=^[0-7][0-9A-HJKMNP-TV-Za-hjkmnp-tv-z]{25}$"`
	// Human-readable name for the resource. Between 1 and 256 characters.
	Name string `json:"name"`
	// Optional description of the resource. Maximum 1024 characters.
	Description *string `json:"description,omitempty"`
	// Additional metadata for the resource.
	Metadata map[string]string `json:"metadata,omitempty"`
	// Timestamp of when the resource was created.
	CreatedAt time.Time `json:"createdAt"`
	// Timestamp of when the resource was last updated.
	UpdatedAt time.Time `json:"updatedAt"`
	// Timestamp of when the resource was permanently deleted.
	DeletedAt *time.Time `json:"deletedAt,omitempty"`
	// The cadence start of the resource.
	ActiveFrom time.Time `json:"activeFrom"`
	// The cadence end of the resource.
	ActiveTo *time.Time `json:"activeTo,omitempty"`
	// Set of key-value pairs managed by the system. Cannot be modified by user.
	Annotations map[string]interface{} `json:"annotations,omitempty"`
	// The status of the subscription.
	Status SubscriptionStatus `json:"status"`
	// The customer ID of the subscription.
	CustomerId string `json:"customerId" validate:"regexp=^[0-7][0-9A-HJKMNP-TV-Za-hjkmnp-tv-z]{25}$"`
	// The plan of the subscription.
	Plan *PlanReference `json:"plan,omitempty"`
	// The currency code of the subscription. Will be revised once we add multi currency support.
	Currency string `json:"currency" validate:"regexp=^[A-Z]{3}$"`
	// The billing cadence for the subscriptions. Defines how often customers are billed using ISO8601 duration format. Examples: \"P1M\" (monthly), \"P3M\" (quarterly), \"P1Y\" (annually).
	BillingCadence string `json:"billingCadence"`
	// The pro-rating configuration for the subscriptions.
	ProRatingConfig *ProRatingConfig `json:"proRatingConfig,omitempty"`
	// The normalizedbilling anchor of the subscription.
	BillingAnchor time.Time `json:"billingAnchor"`
	// Commercial terms for this subscription (discounts, pool overrides, wallet policy).
	CommercialOverrides *CommercialOverrides `json:"commercialOverrides,omitempty"`
	// Idempotency key linking this subscription to the accepted quote version that created it.
	QuoteOriginKey *string `json:"quoteOriginKey,omitempty" validate:"regexp=^\\\\S(?:.*\\\\S)?$"`
	// Transient grant activation status returned by create and quote-origin duplicate recovery flows.
	GrantsStatus *string `json:"grantsStatus,omitempty"`
	// Alignment details enriched with the current billing period.
	Alignment *SubscriptionAlignment `json:"alignment,omitempty"`
	// The phases of the subscription.
	Phases []SubscriptionPhaseExpanded `json:"phases"`
}

type _SubscriptionExpanded SubscriptionExpanded

// NewSubscriptionExpanded instantiates a new SubscriptionExpanded object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewSubscriptionExpanded(id string, name string, createdAt time.Time, updatedAt time.Time, activeFrom time.Time, status SubscriptionStatus, customerId string, currency string, billingCadence string, billingAnchor time.Time, phases []SubscriptionPhaseExpanded) *SubscriptionExpanded {
	this := SubscriptionExpanded{}
	this.Id = id
	this.Name = name
	this.CreatedAt = createdAt
	this.UpdatedAt = updatedAt
	this.ActiveFrom = activeFrom
	this.Status = status
	this.CustomerId = customerId
	this.Currency = currency
	this.BillingCadence = billingCadence
	this.BillingAnchor = billingAnchor
	this.Phases = phases
	return &this
}

// NewSubscriptionExpandedWithDefaults instantiates a new SubscriptionExpanded object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewSubscriptionExpandedWithDefaults() *SubscriptionExpanded {
	this := SubscriptionExpanded{}
	return &this
}

// GetId returns the Id field value
func (o *SubscriptionExpanded) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *SubscriptionExpanded) SetId(v string) {
	o.Id = v
}

// GetName returns the Name field value
func (o *SubscriptionExpanded) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *SubscriptionExpanded) SetName(v string) {
	o.Name = v
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *SubscriptionExpanded) SetDescription(v string) {
	o.Description = &v
}

// GetMetadata returns the Metadata field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetMetadata() map[string]string {
	if o == nil || IsNil(o.Metadata) {
		var ret map[string]string
		return ret
	}
	return o.Metadata
}

// GetMetadataOk returns a tuple with the Metadata field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetMetadataOk() (map[string]string, bool) {
	if o == nil || IsNil(o.Metadata) {
		return map[string]string{}, false
	}
	return o.Metadata, true
}

// HasMetadata returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasMetadata() bool {
	if o != nil && !IsNil(o.Metadata) {
		return true
	}

	return false
}

// SetMetadata gets a reference to the given map[string]string and assigns it to the Metadata field.
func (o *SubscriptionExpanded) SetMetadata(v map[string]string) {
	o.Metadata = v
}

// GetCreatedAt returns the CreatedAt field value
func (o *SubscriptionExpanded) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *SubscriptionExpanded) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

// GetUpdatedAt returns the UpdatedAt field value
func (o *SubscriptionExpanded) GetUpdatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.UpdatedAt
}

// GetUpdatedAtOk returns a tuple with the UpdatedAt field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetUpdatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.UpdatedAt, true
}

// SetUpdatedAt sets field value
func (o *SubscriptionExpanded) SetUpdatedAt(v time.Time) {
	o.UpdatedAt = v
}

// GetDeletedAt returns the DeletedAt field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetDeletedAt() time.Time {
	if o == nil || IsNil(o.DeletedAt) {
		var ret time.Time
		return ret
	}
	return *o.DeletedAt
}

// GetDeletedAtOk returns a tuple with the DeletedAt field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetDeletedAtOk() (*time.Time, bool) {
	if o == nil || IsNil(o.DeletedAt) {
		return nil, false
	}
	return o.DeletedAt, true
}

// HasDeletedAt returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasDeletedAt() bool {
	if o != nil && !IsNil(o.DeletedAt) {
		return true
	}

	return false
}

// SetDeletedAt gets a reference to the given time.Time and assigns it to the DeletedAt field.
func (o *SubscriptionExpanded) SetDeletedAt(v time.Time) {
	o.DeletedAt = &v
}

// GetActiveFrom returns the ActiveFrom field value
func (o *SubscriptionExpanded) GetActiveFrom() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.ActiveFrom
}

// GetActiveFromOk returns a tuple with the ActiveFrom field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetActiveFromOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ActiveFrom, true
}

// SetActiveFrom sets field value
func (o *SubscriptionExpanded) SetActiveFrom(v time.Time) {
	o.ActiveFrom = v
}

// GetActiveTo returns the ActiveTo field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetActiveTo() time.Time {
	if o == nil || IsNil(o.ActiveTo) {
		var ret time.Time
		return ret
	}
	return *o.ActiveTo
}

// GetActiveToOk returns a tuple with the ActiveTo field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetActiveToOk() (*time.Time, bool) {
	if o == nil || IsNil(o.ActiveTo) {
		return nil, false
	}
	return o.ActiveTo, true
}

// HasActiveTo returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasActiveTo() bool {
	if o != nil && !IsNil(o.ActiveTo) {
		return true
	}

	return false
}

// SetActiveTo gets a reference to the given time.Time and assigns it to the ActiveTo field.
func (o *SubscriptionExpanded) SetActiveTo(v time.Time) {
	o.ActiveTo = &v
}

// GetAnnotations returns the Annotations field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetAnnotations() map[string]interface{} {
	if o == nil || IsNil(o.Annotations) {
		var ret map[string]interface{}
		return ret
	}
	return o.Annotations
}

// GetAnnotationsOk returns a tuple with the Annotations field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetAnnotationsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.Annotations) {
		return map[string]interface{}{}, false
	}
	return o.Annotations, true
}

// HasAnnotations returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasAnnotations() bool {
	if o != nil && !IsNil(o.Annotations) {
		return true
	}

	return false
}

// SetAnnotations gets a reference to the given map[string]interface{} and assigns it to the Annotations field.
func (o *SubscriptionExpanded) SetAnnotations(v map[string]interface{}) {
	o.Annotations = v
}

// GetStatus returns the Status field value
func (o *SubscriptionExpanded) GetStatus() SubscriptionStatus {
	if o == nil {
		var ret SubscriptionStatus
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetStatusOk() (*SubscriptionStatus, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *SubscriptionExpanded) SetStatus(v SubscriptionStatus) {
	o.Status = v
}

// GetCustomerId returns the CustomerId field value
func (o *SubscriptionExpanded) GetCustomerId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.CustomerId
}

// GetCustomerIdOk returns a tuple with the CustomerId field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetCustomerIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CustomerId, true
}

// SetCustomerId sets field value
func (o *SubscriptionExpanded) SetCustomerId(v string) {
	o.CustomerId = v
}

// GetPlan returns the Plan field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetPlan() PlanReference {
	if o == nil || IsNil(o.Plan) {
		var ret PlanReference
		return ret
	}
	return *o.Plan
}

// GetPlanOk returns a tuple with the Plan field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetPlanOk() (*PlanReference, bool) {
	if o == nil || IsNil(o.Plan) {
		return nil, false
	}
	return o.Plan, true
}

// HasPlan returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasPlan() bool {
	if o != nil && !IsNil(o.Plan) {
		return true
	}

	return false
}

// SetPlan gets a reference to the given PlanReference and assigns it to the Plan field.
func (o *SubscriptionExpanded) SetPlan(v PlanReference) {
	o.Plan = &v
}

// GetCurrency returns the Currency field value
func (o *SubscriptionExpanded) GetCurrency() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Currency
}

// GetCurrencyOk returns a tuple with the Currency field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetCurrencyOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Currency, true
}

// SetCurrency sets field value
func (o *SubscriptionExpanded) SetCurrency(v string) {
	o.Currency = v
}

// GetBillingCadence returns the BillingCadence field value
func (o *SubscriptionExpanded) GetBillingCadence() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.BillingCadence
}

// GetBillingCadenceOk returns a tuple with the BillingCadence field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetBillingCadenceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.BillingCadence, true
}

// SetBillingCadence sets field value
func (o *SubscriptionExpanded) SetBillingCadence(v string) {
	o.BillingCadence = v
}

// GetProRatingConfig returns the ProRatingConfig field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetProRatingConfig() ProRatingConfig {
	if o == nil || IsNil(o.ProRatingConfig) {
		var ret ProRatingConfig
		return ret
	}
	return *o.ProRatingConfig
}

// GetProRatingConfigOk returns a tuple with the ProRatingConfig field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetProRatingConfigOk() (*ProRatingConfig, bool) {
	if o == nil || IsNil(o.ProRatingConfig) {
		return nil, false
	}
	return o.ProRatingConfig, true
}

// HasProRatingConfig returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasProRatingConfig() bool {
	if o != nil && !IsNil(o.ProRatingConfig) {
		return true
	}

	return false
}

// SetProRatingConfig gets a reference to the given ProRatingConfig and assigns it to the ProRatingConfig field.
func (o *SubscriptionExpanded) SetProRatingConfig(v ProRatingConfig) {
	o.ProRatingConfig = &v
}

// GetBillingAnchor returns the BillingAnchor field value
func (o *SubscriptionExpanded) GetBillingAnchor() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.BillingAnchor
}

// GetBillingAnchorOk returns a tuple with the BillingAnchor field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetBillingAnchorOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.BillingAnchor, true
}

// SetBillingAnchor sets field value
func (o *SubscriptionExpanded) SetBillingAnchor(v time.Time) {
	o.BillingAnchor = v
}

// GetCommercialOverrides returns the CommercialOverrides field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetCommercialOverrides() CommercialOverrides {
	if o == nil || IsNil(o.CommercialOverrides) {
		var ret CommercialOverrides
		return ret
	}
	return *o.CommercialOverrides
}

// GetCommercialOverridesOk returns a tuple with the CommercialOverrides field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetCommercialOverridesOk() (*CommercialOverrides, bool) {
	if o == nil || IsNil(o.CommercialOverrides) {
		return nil, false
	}
	return o.CommercialOverrides, true
}

// HasCommercialOverrides returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasCommercialOverrides() bool {
	if o != nil && !IsNil(o.CommercialOverrides) {
		return true
	}

	return false
}

// SetCommercialOverrides gets a reference to the given CommercialOverrides and assigns it to the CommercialOverrides field.
func (o *SubscriptionExpanded) SetCommercialOverrides(v CommercialOverrides) {
	o.CommercialOverrides = &v
}

// GetQuoteOriginKey returns the QuoteOriginKey field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetQuoteOriginKey() string {
	if o == nil || IsNil(o.QuoteOriginKey) {
		var ret string
		return ret
	}
	return *o.QuoteOriginKey
}

// GetQuoteOriginKeyOk returns a tuple with the QuoteOriginKey field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetQuoteOriginKeyOk() (*string, bool) {
	if o == nil || IsNil(o.QuoteOriginKey) {
		return nil, false
	}
	return o.QuoteOriginKey, true
}

// HasQuoteOriginKey returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasQuoteOriginKey() bool {
	if o != nil && !IsNil(o.QuoteOriginKey) {
		return true
	}

	return false
}

// SetQuoteOriginKey gets a reference to the given string and assigns it to the QuoteOriginKey field.
func (o *SubscriptionExpanded) SetQuoteOriginKey(v string) {
	o.QuoteOriginKey = &v
}

// GetGrantsStatus returns the GrantsStatus field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetGrantsStatus() string {
	if o == nil || IsNil(o.GrantsStatus) {
		var ret string
		return ret
	}
	return *o.GrantsStatus
}

// GetGrantsStatusOk returns a tuple with the GrantsStatus field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetGrantsStatusOk() (*string, bool) {
	if o == nil || IsNil(o.GrantsStatus) {
		return nil, false
	}
	return o.GrantsStatus, true
}

// HasGrantsStatus returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasGrantsStatus() bool {
	if o != nil && !IsNil(o.GrantsStatus) {
		return true
	}

	return false
}

// SetGrantsStatus gets a reference to the given string and assigns it to the GrantsStatus field.
func (o *SubscriptionExpanded) SetGrantsStatus(v string) {
	o.GrantsStatus = &v
}

// GetAlignment returns the Alignment field value if set, zero value otherwise.
func (o *SubscriptionExpanded) GetAlignment() SubscriptionAlignment {
	if o == nil || IsNil(o.Alignment) {
		var ret SubscriptionAlignment
		return ret
	}
	return *o.Alignment
}

// GetAlignmentOk returns a tuple with the Alignment field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetAlignmentOk() (*SubscriptionAlignment, bool) {
	if o == nil || IsNil(o.Alignment) {
		return nil, false
	}
	return o.Alignment, true
}

// HasAlignment returns a boolean if a field has been set.
func (o *SubscriptionExpanded) HasAlignment() bool {
	if o != nil && !IsNil(o.Alignment) {
		return true
	}

	return false
}

// SetAlignment gets a reference to the given SubscriptionAlignment and assigns it to the Alignment field.
func (o *SubscriptionExpanded) SetAlignment(v SubscriptionAlignment) {
	o.Alignment = &v
}

// GetPhases returns the Phases field value
func (o *SubscriptionExpanded) GetPhases() []SubscriptionPhaseExpanded {
	if o == nil {
		var ret []SubscriptionPhaseExpanded
		return ret
	}

	return o.Phases
}

// GetPhasesOk returns a tuple with the Phases field value
// and a boolean to check if the value has been set.
func (o *SubscriptionExpanded) GetPhasesOk() ([]SubscriptionPhaseExpanded, bool) {
	if o == nil {
		return nil, false
	}
	return o.Phases, true
}

// SetPhases sets field value
func (o *SubscriptionExpanded) SetPhases(v []SubscriptionPhaseExpanded) {
	o.Phases = v
}

func (o SubscriptionExpanded) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o SubscriptionExpanded) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["name"] = o.Name
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if !IsNil(o.Metadata) {
		toSerialize["metadata"] = o.Metadata
	}
	toSerialize["createdAt"] = o.CreatedAt
	toSerialize["updatedAt"] = o.UpdatedAt
	if !IsNil(o.DeletedAt) {
		toSerialize["deletedAt"] = o.DeletedAt
	}
	toSerialize["activeFrom"] = o.ActiveFrom
	if !IsNil(o.ActiveTo) {
		toSerialize["activeTo"] = o.ActiveTo
	}
	if !IsNil(o.Annotations) {
		toSerialize["annotations"] = o.Annotations
	}
	toSerialize["status"] = o.Status
	toSerialize["customerId"] = o.CustomerId
	if !IsNil(o.Plan) {
		toSerialize["plan"] = o.Plan
	}
	toSerialize["currency"] = o.Currency
	toSerialize["billingCadence"] = o.BillingCadence
	if !IsNil(o.ProRatingConfig) {
		toSerialize["proRatingConfig"] = o.ProRatingConfig
	}
	toSerialize["billingAnchor"] = o.BillingAnchor
	if !IsNil(o.CommercialOverrides) {
		toSerialize["commercialOverrides"] = o.CommercialOverrides
	}
	if !IsNil(o.QuoteOriginKey) {
		toSerialize["quoteOriginKey"] = o.QuoteOriginKey
	}
	if !IsNil(o.GrantsStatus) {
		toSerialize["grantsStatus"] = o.GrantsStatus
	}
	if !IsNil(o.Alignment) {
		toSerialize["alignment"] = o.Alignment
	}
	toSerialize["phases"] = o.Phases
	return toSerialize, nil
}

func (o *SubscriptionExpanded) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"name",
		"createdAt",
		"updatedAt",
		"activeFrom",
		"status",
		"customerId",
		"currency",
		"billingCadence",
		"billingAnchor",
		"phases",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varSubscriptionExpanded := _SubscriptionExpanded{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varSubscriptionExpanded)

	if err != nil {
		return err
	}

	*o = SubscriptionExpanded(varSubscriptionExpanded)

	return err
}

type NullableSubscriptionExpanded struct {
	value *SubscriptionExpanded
	isSet bool
}

func (v NullableSubscriptionExpanded) Get() *SubscriptionExpanded {
	return v.value
}

func (v *NullableSubscriptionExpanded) Set(val *SubscriptionExpanded) {
	v.value = val
	v.isSet = true
}

func (v NullableSubscriptionExpanded) IsSet() bool {
	return v.isSet
}

func (v *NullableSubscriptionExpanded) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableSubscriptionExpanded(val *SubscriptionExpanded) *NullableSubscriptionExpanded {
	return &NullableSubscriptionExpanded{value: val, isSet: true}
}

func (v NullableSubscriptionExpanded) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableSubscriptionExpanded) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


