/*
Moolabs API

Unified Moolabs SDK. Capability-based public surface stitched from internal service specs: bff, meter, arc, acute. Customers see capability namespaces (usage, billing, collections, etc.); the SDK routes per-operation internally.

API version: 0.3.0-rc2
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package moolabs

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the QuoteAgentProvenanceRunResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &QuoteAgentProvenanceRunResponse{}

// QuoteAgentProvenanceRunResponse struct for QuoteAgentProvenanceRunResponse
type QuoteAgentProvenanceRunResponse struct {
	Id string `json:"id"`
	TenantId string `json:"tenant_id"`
	QuoteId string `json:"quote_id"`
	QuoteVersion int32 `json:"quote_version"`
	AgentName string `json:"agent_name"`
	TrustTier int32 `json:"trust_tier"`
	Surface string `json:"surface"`
	ActorType string `json:"actor_type"`
	ActorId string `json:"actor_id"`
	Status string `json:"status"`
	StartedAt time.Time `json:"started_at"`
	CompletedAt time.Time `json:"completed_at"`
	RationaleSummary string `json:"rationale_summary"`
	ErrorCode string `json:"error_code"`
}

type _QuoteAgentProvenanceRunResponse QuoteAgentProvenanceRunResponse

// NewQuoteAgentProvenanceRunResponse instantiates a new QuoteAgentProvenanceRunResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewQuoteAgentProvenanceRunResponse(id string, tenantId string, quoteId string, quoteVersion int32, agentName string, trustTier int32, surface string, actorType string, actorId string, status string, startedAt time.Time, completedAt time.Time, rationaleSummary string, errorCode string) *QuoteAgentProvenanceRunResponse {
	this := QuoteAgentProvenanceRunResponse{}
	this.Id = id
	this.TenantId = tenantId
	this.QuoteId = quoteId
	this.QuoteVersion = quoteVersion
	this.AgentName = agentName
	this.TrustTier = trustTier
	this.Surface = surface
	this.ActorType = actorType
	this.ActorId = actorId
	this.Status = status
	this.StartedAt = startedAt
	this.CompletedAt = completedAt
	this.RationaleSummary = rationaleSummary
	this.ErrorCode = errorCode
	return &this
}

// NewQuoteAgentProvenanceRunResponseWithDefaults instantiates a new QuoteAgentProvenanceRunResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewQuoteAgentProvenanceRunResponseWithDefaults() *QuoteAgentProvenanceRunResponse {
	this := QuoteAgentProvenanceRunResponse{}
	return &this
}

// GetId returns the Id field value
func (o *QuoteAgentProvenanceRunResponse) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *QuoteAgentProvenanceRunResponse) SetId(v string) {
	o.Id = v
}

// GetTenantId returns the TenantId field value
func (o *QuoteAgentProvenanceRunResponse) GetTenantId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.TenantId
}

// GetTenantIdOk returns a tuple with the TenantId field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetTenantIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TenantId, true
}

// SetTenantId sets field value
func (o *QuoteAgentProvenanceRunResponse) SetTenantId(v string) {
	o.TenantId = v
}

// GetQuoteId returns the QuoteId field value
func (o *QuoteAgentProvenanceRunResponse) GetQuoteId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.QuoteId
}

// GetQuoteIdOk returns a tuple with the QuoteId field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetQuoteIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.QuoteId, true
}

// SetQuoteId sets field value
func (o *QuoteAgentProvenanceRunResponse) SetQuoteId(v string) {
	o.QuoteId = v
}

// GetQuoteVersion returns the QuoteVersion field value
func (o *QuoteAgentProvenanceRunResponse) GetQuoteVersion() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.QuoteVersion
}

// GetQuoteVersionOk returns a tuple with the QuoteVersion field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetQuoteVersionOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.QuoteVersion, true
}

// SetQuoteVersion sets field value
func (o *QuoteAgentProvenanceRunResponse) SetQuoteVersion(v int32) {
	o.QuoteVersion = v
}

// GetAgentName returns the AgentName field value
func (o *QuoteAgentProvenanceRunResponse) GetAgentName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AgentName
}

// GetAgentNameOk returns a tuple with the AgentName field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetAgentNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AgentName, true
}

// SetAgentName sets field value
func (o *QuoteAgentProvenanceRunResponse) SetAgentName(v string) {
	o.AgentName = v
}

// GetTrustTier returns the TrustTier field value
func (o *QuoteAgentProvenanceRunResponse) GetTrustTier() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.TrustTier
}

// GetTrustTierOk returns a tuple with the TrustTier field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetTrustTierOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.TrustTier, true
}

// SetTrustTier sets field value
func (o *QuoteAgentProvenanceRunResponse) SetTrustTier(v int32) {
	o.TrustTier = v
}

// GetSurface returns the Surface field value
func (o *QuoteAgentProvenanceRunResponse) GetSurface() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Surface
}

// GetSurfaceOk returns a tuple with the Surface field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetSurfaceOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Surface, true
}

// SetSurface sets field value
func (o *QuoteAgentProvenanceRunResponse) SetSurface(v string) {
	o.Surface = v
}

// GetActorType returns the ActorType field value
func (o *QuoteAgentProvenanceRunResponse) GetActorType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ActorType
}

// GetActorTypeOk returns a tuple with the ActorType field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetActorTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ActorType, true
}

// SetActorType sets field value
func (o *QuoteAgentProvenanceRunResponse) SetActorType(v string) {
	o.ActorType = v
}

// GetActorId returns the ActorId field value
func (o *QuoteAgentProvenanceRunResponse) GetActorId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ActorId
}

// GetActorIdOk returns a tuple with the ActorId field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetActorIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ActorId, true
}

// SetActorId sets field value
func (o *QuoteAgentProvenanceRunResponse) SetActorId(v string) {
	o.ActorId = v
}

// GetStatus returns the Status field value
func (o *QuoteAgentProvenanceRunResponse) GetStatus() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Status
}

// GetStatusOk returns a tuple with the Status field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetStatusOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Status, true
}

// SetStatus sets field value
func (o *QuoteAgentProvenanceRunResponse) SetStatus(v string) {
	o.Status = v
}

// GetStartedAt returns the StartedAt field value
func (o *QuoteAgentProvenanceRunResponse) GetStartedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.StartedAt
}

// GetStartedAtOk returns a tuple with the StartedAt field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetStartedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.StartedAt, true
}

// SetStartedAt sets field value
func (o *QuoteAgentProvenanceRunResponse) SetStartedAt(v time.Time) {
	o.StartedAt = v
}

// GetCompletedAt returns the CompletedAt field value
func (o *QuoteAgentProvenanceRunResponse) GetCompletedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CompletedAt
}

// GetCompletedAtOk returns a tuple with the CompletedAt field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetCompletedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CompletedAt, true
}

// SetCompletedAt sets field value
func (o *QuoteAgentProvenanceRunResponse) SetCompletedAt(v time.Time) {
	o.CompletedAt = v
}

// GetRationaleSummary returns the RationaleSummary field value
func (o *QuoteAgentProvenanceRunResponse) GetRationaleSummary() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.RationaleSummary
}

// GetRationaleSummaryOk returns a tuple with the RationaleSummary field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetRationaleSummaryOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.RationaleSummary, true
}

// SetRationaleSummary sets field value
func (o *QuoteAgentProvenanceRunResponse) SetRationaleSummary(v string) {
	o.RationaleSummary = v
}

// GetErrorCode returns the ErrorCode field value
func (o *QuoteAgentProvenanceRunResponse) GetErrorCode() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ErrorCode
}

// GetErrorCodeOk returns a tuple with the ErrorCode field value
// and a boolean to check if the value has been set.
func (o *QuoteAgentProvenanceRunResponse) GetErrorCodeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ErrorCode, true
}

// SetErrorCode sets field value
func (o *QuoteAgentProvenanceRunResponse) SetErrorCode(v string) {
	o.ErrorCode = v
}

func (o QuoteAgentProvenanceRunResponse) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o QuoteAgentProvenanceRunResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["tenant_id"] = o.TenantId
	toSerialize["quote_id"] = o.QuoteId
	toSerialize["quote_version"] = o.QuoteVersion
	toSerialize["agent_name"] = o.AgentName
	toSerialize["trust_tier"] = o.TrustTier
	toSerialize["surface"] = o.Surface
	toSerialize["actor_type"] = o.ActorType
	toSerialize["actor_id"] = o.ActorId
	toSerialize["status"] = o.Status
	toSerialize["started_at"] = o.StartedAt
	toSerialize["completed_at"] = o.CompletedAt
	toSerialize["rationale_summary"] = o.RationaleSummary
	toSerialize["error_code"] = o.ErrorCode
	return toSerialize, nil
}

func (o *QuoteAgentProvenanceRunResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"tenant_id",
		"quote_id",
		"quote_version",
		"agent_name",
		"trust_tier",
		"surface",
		"actor_type",
		"actor_id",
		"status",
		"started_at",
		"completed_at",
		"rationale_summary",
		"error_code",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varQuoteAgentProvenanceRunResponse := _QuoteAgentProvenanceRunResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varQuoteAgentProvenanceRunResponse)

	if err != nil {
		return err
	}

	*o = QuoteAgentProvenanceRunResponse(varQuoteAgentProvenanceRunResponse)

	return err
}

type NullableQuoteAgentProvenanceRunResponse struct {
	value *QuoteAgentProvenanceRunResponse
	isSet bool
}

func (v NullableQuoteAgentProvenanceRunResponse) Get() *QuoteAgentProvenanceRunResponse {
	return v.value
}

func (v *NullableQuoteAgentProvenanceRunResponse) Set(val *QuoteAgentProvenanceRunResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableQuoteAgentProvenanceRunResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableQuoteAgentProvenanceRunResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableQuoteAgentProvenanceRunResponse(val *QuoteAgentProvenanceRunResponse) *NullableQuoteAgentProvenanceRunResponse {
	return &NullableQuoteAgentProvenanceRunResponse{value: val, isSet: true}
}

func (v NullableQuoteAgentProvenanceRunResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableQuoteAgentProvenanceRunResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


